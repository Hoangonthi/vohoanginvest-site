...
function render(data){
  const c=data.customer||{}; const s=data.crm_state||{}; const a=data.latest_assessment||{};
  ...
  content.innerHTML=`
    <section class="hero">
      <article class="card">
        ...
        <div class="chips"><span class="chip">${esc(priority)}</span><span class="chip">${esc(labelStage(s.stage))}</span>${a.severity?`<span class="chip">${esc(labelSeverity(a.severity))}</span>`:''}</div>
      ...
      <article class="card"><h2 class="section-title">Việc nên làm tiếp</h2><strong>${esc(next.title||'Chưa có việc cấp thiết')}</strong><p class="small">${esc(s.computed_priority?.reason||'')}</p><p>Hạn: ${fmt(next.due_at)}</p>${next.task_id?`<button class="primary" data-complete-task="${esc(next.task_id)}">Đánh dấu đã xong</button>`:''}</article>
    </section>
    ...
    <section class="grid">
      ...
      <article class="card"><h2 class="section-title">Trạng thái chăm sóc</h2><p>Giai đoạn: <strong>${esc(labelStage(s.stage))}</strong></p><p>Lần liên hệ gần nhất: ${fmt(s.last_contact_at)}</p><p>Người phụ trách: ${esc(s.owner_name||'Chưa phân công')}</p><label>Đổi giai đoạn<select id="stageSelect"><option value="NEW_LEAD">Lead mới</option><option value="ENGAGED">Đã tương tác</option><option value="QUALIFIED">Đủ điều kiện</option><option value="MEETING">Đang hẹn</option><option value="CLIENT">Khách hàng</option><option value="NURTURE">Theo dõi</option><option value="LOST">Không tiếp tục</option></select></label><button class="primary" id="saveStage" style="margin-top:8px">Lưu giai đoạn</button></article>
    </section>
...
async function load(){
  ...
  const {data,error}=await supabaseClient.rpc('admin_get_customer_360_v11',{p_customer_id:customerId});
...
async function addActivity(type,note){
  const text=(note||prompt(type==='NOTE'?'Ghi chú:':'Nội dung trao đổi:')||'').trim(); if(!text)return;
  const {error}=await supabaseClient.rpc('admin_add_activity',{p_customer_id:customerId,p_activity_type:type,p_note:text,p_meeting_request_id:null});
  if(error){alert('Chưa lưu được: '+error.message);return} await load();
}

content.addEventListener('click',async e=>{
  const task=e.target.closest('[data-complete-task]');
  if(task){task.disabled=true;const {error}=await supabaseClient.rpc('admin_complete_task',{p_task_id:task.dataset.completeTask});if(error)alert('Chưa hoàn thành task: '+error.message);await load();return}
  ...
  if(e.target.id==='saveStage'){const v=document.querySelector('#stageSelect').value;const {error}=await supabaseClient.rpc('admin_update_customer_stage',{p_customer_id:customerId,p_stage:v});if(error)alert('Chưa đổi được giai đoạn: '+error.message);await load();}
});
