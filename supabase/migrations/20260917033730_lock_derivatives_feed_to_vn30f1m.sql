-- Hard-lock the public derivatives card to VN30F1M only.
-- Prevent VN-Index, VN30 cash, or any other symbol from overwriting psvn_trend.
...
begin
  ...
  if v_symbol <> 'VN30F1M' then raise exception 'DERIVATIVES_SYMBOL_NOT_ALLOWED'; end if;
...
as $$
  select coalesce((
    ...
    where s.id='psvn_trend'
      and upper(coalesce(s.payload->>'symbol',''))='VN30F1M'
  ), jsonb_build_object(
    'title','Xu hướng phái sinh',
    'symbol',null,
    'trend',null,
